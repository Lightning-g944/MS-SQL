...
select distinct(d1.State) 
		from FACT_TRANSACTIONS F1 
	inner join 
		DIM_LOCATION D1 on f1.IDLocation = D1.IDLocation
where YEAR(F1.Date) >= 2005



--2. Which state is the US is buying  more 'Samsung' cell phones?
...
select distinct(D1.State) 
	...
	INNER JOIN
		DIM_LOCATION D1 on F1.IDLocation = D1.IDLocation
	INNER JOIN
		DIM_MODEL M1 on F1.IDModel = M1.IDModel
	INNER JOIN
		DIM_MANUFACTURER D2 on m1.IDManufacturer = D2.IDManufacturer

	where d1.Country = 'US' and D2.Manufacturer_Name = 'SAMSUNG'



--3. Show the number of transactions for each model per zip code per state.

select M1.Model_Name, L1.State, L1.ZipCode, COUNT(F1.Quantity) [Transactions]
		from FACT_TRANSACTIONS F1
	INNER JOIN
		DIM_MODEL M1 on F1.IDModel = M1.IDModel
	INNER JOIN
		DIM_LOCATION L1 ON  F1.IDLocation = L1.IDLocation

group by M1.Model_Name, L1.State, L1.ZipCode


--4. Show the Cheapest Cellphone

select Top 1 M1.Model_Name, F1.TotalPrice [Unit price]
		from FACT_TRANSACTIONS F1
	inner join
		DIM_MODEL M1 on F1.IDModel = M1.IDModel
	order by F1.TotalPrice
...
select Top 5 MF.Manufacturer_Name,M1.Model_Name, AVG(F1.TotalPrice) [Average_Price] from 
		FACT_TRANSACTIONS F1
	inner join
		DIM_MODEL M1 on F1.IDModel = M1.IDModel
	inner join
		DIM_MANUFACTURER MF on MF.IDManufacturer = M1.IDManufacturer
	group by MF.Manufacturer_Name, M1.Model_Name
	order by AVG(F1.TotalPrice) desc
...
select C1.Customer_Name, AVG(F1.TotalPrice) [Average_Amount_Spent]
		from FACT_TRANSACTIONS F1
	inner join
		DIM_CUSTOMER C1 on C1.IDCustomer = F1.IDCustomer
	inner join
		DIM_DATE D1 on F1.Date = D1.DATE
	where YEAR(F1.Date) = 2009
	group by C1.Customer_Name
	having AVG(F1.TotalPrice) > 500


--7. List if there is any model that was in the top 5 in terms of quantity,
--		simultaneously in 2008,2009 2010
...
select * from 
	(
		select * from 
				(
				select Top 5 M1.Model_Name 
				from DIM_MODEL M1 
				left join FACT_TRANSACTIONS F1 
				on M1.IDModel = F1.IDModel
				where YEAR(F1.Date) = 2008
				group by M1.Model_Name
				order by SUM(F1.Quantity) desc)A
		union all
			select * from 
			(
				select Top 5 M1.Model_Name 
				...
				where YEAR(F1.Date) = 2009
				group by M1.Model_Name
				order by SUM(F1.Quantity) desc)B
		union all
			select * from 
			(
				select Top 5 M1.Model_Name 
				...
				left join FACT_TRANSACTIONS F1 
				on M1.IDModel = F1.IDModel
				where YEAR(F1.Date) = 2010
				group by M1.Model_Name
				order by SUM(F1.Quantity) desc)C
	) as Q7
group by Q7.Model_Name
...
select * 
			from DIM_MANUFACTURER M1 
		left join 
			DIM_MODEL M2 on M1.IDManufacturer = M2.IDManufacturer 
		left join 
			FACT_TRANSACTIONS F1 on M2.IDModel = F1.IDModel
		where (YEAR(F1.Date) = 2009)
		AND F1.TotalPrice = ( select MAX(TotalPrice) from FACT_TRANSACTIONS F1
							  where YEAR(Date) = 2009 and TotalPrice NOT IN 
							 (select MAX(TotalPrice) from FACT_TRANSACTIONS where YEAR(Date) = 2009)
							)
			OR (YEAR(F1.Date) = 2010)
		AND F1.TotalPrice = ( select MAX(TotalPrice) from FACT_TRANSACTIONS F1
							  where YEAR(Date) = 2010 and TotalPrice NOT IN 
							 (select MAX(TotalPrice) from FACT_TRANSACTIONS where YEAR(Date) = 2010)
							)
...
select DISTINCT(M2.Manufacturer_Name)
			from 
				FACT_TRANSACTIONS F1
			inner join
				DIM_MODEL M1 on F1.IDModel = M1.IDModel
	...
	where YEAR(F1.Date) = 2010 AND YEAR(F1.Date) != 2009
...
select top 100 C1.Customer_Name,
		AVG(CASE WHEN YEAR(F1.Date) = 2003 THEN F1.TotalPrice END) AS AVG_SPENT_IN_2003,
		AVG(CASE WHEN YEAR(F1.Date) = 2003 THEN F1.Quantity END) AS AVG_QTY_IN_2003,
		AVG(CASE WHEN YEAR(F1.Date) = 2004 THEN F1.TotalPrice END) AS AVG_SPENT_IN_2004,
		AVG(CASE WHEN YEAR(F1.Date) = 2004 THEN F1.Quantity END) AS AVG_QTY_IN_2004,
		AVG(CASE WHEN YEAR(F1.Date) = 2005 THEN F1.TotalPrice END) AS AVG_SPENT_IN_2005,
		AVG(CASE WHEN YEAR(F1.Date) = 2005 THEN F1.Quantity END) AS AVG_QTY_IN_2005,
		AVG(CASE WHEN YEAR(F1.Date) = 2006 THEN F1.TotalPrice END) AS AVG_SPENT_IN_2006,
		AVG(CASE WHEN YEAR(F1.Date) = 2006 THEN F1.Quantity END) AS AVG_QTY_IN_2006,
		AVG(CASE WHEN YEAR(F1.Date) = 2007 THEN F1.TotalPrice END) AS AVG_SPENT_IN_2007,
		AVG(CASE WHEN YEAR(F1.Date) = 2007 THEN F1.Quantity END) AS AVG_QTY_IN_2007,
		AVG(CASE WHEN YEAR(F1.Date) = 2008 THEN F1.TotalPrice END) AS AVG_SPENT_IN_2008,
		AVG(CASE WHEN YEAR(F1.Date) = 2008 THEN F1.Quantity END) AS AVG_QTY_IN_2008,
		AVG(CASE WHEN YEAR(F1.Date) = 2009 THEN F1.TotalPrice END) AS AVG_SPENT_IN_2009,
		AVG(CASE WHEN YEAR(F1.Date) = 2009 THEN F1.Quantity END) AS AVG_QTY_IN_2009,
		AVG(CASE WHEN YEAR(F1.Date) = 2010 THEN F1.TotalPrice END) AS AVG_SPENT_IN_2010,
		AVG(CASE WHEN YEAR(F1.Date) = 2010 THEN F1.Quantity END) AS AVG_QTY_IN_2010
			from
				FACT_TRANSACTIONS F1
			left join
				DIM_CUSTOMER C1 on F1.IDCustomer = C1.IDCustomer
		GROUP BY C1.Customer_Name
		ORDER BY SUM(F1.TotalPrice) DESC
